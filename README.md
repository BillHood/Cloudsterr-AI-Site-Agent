...
Current application version: `v0.0.27`. Subsequent releases increment the patch number sequentially.
...
Version 0.0.27 records the authenticated HTTPS Firestore Listen GET backchannel as a separate versioned permission from the existing POST read-stream permission. Both permissions require the exact host and `/Listen/channel` path suffix. `/Write/channel` and every other unapproved external endpoint remain blocked. Cloudsterr does not store request or response bodies, headers, queries, page text, screenshots, field values, credentials, tokens, or session data.
