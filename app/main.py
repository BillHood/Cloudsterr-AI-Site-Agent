...
FRED_ONLINE_CHECK = {
    "input_selector": "#fred-chat-input",
    "submit_selector": "form:has(#fred-chat-input) button[type='submit']",
    "message": "Hi Fred, Cloudsterr AI Site Agent - checking in?",
    "firestore_write_confirmed": True,
    "firestore_write_get_confirmed": True,
    "gemini_post_confirmed": True,
...
LOGIN_DEFINITION_FIELDS = (
    ...
    "firestore_listen_get_enabled",
    "revenuecat_subscriber_get_enabled",
)
...
def login_definition(source) -> dict:
    ...
    for field in LOGIN_DEFINITION_FIELDS:
        ...
        if field in {"authenticated_shell_check", "firestore_listen_enabled", "firestore_listen_get_enabled", "revenuecat_subscriber_get_enabled"}:
            value = bool(value)
...
def connect_database() -> sqlite3.Connection:
    ...
    connection.execute(
        ...
        CREATE TABLE IF NOT EXISTS login_journeys (
            ...
            firestore_listen_get_enabled INTEGER NOT NULL DEFAULT 0,
            revenuecat_subscriber_get_enabled INTEGER NOT NULL DEFAULT 0,
    ...
    if "firestore_listen_get_enabled" not in login_journey_columns:
        connection.execute("ALTER TABLE login_journeys ADD COLUMN firestore_listen_get_enabled INTEGER NOT NULL DEFAULT 0")
    if "revenuecat_subscriber_get_enabled" not in login_journey_columns:
        connection.execute("ALTER TABLE login_journeys ADD COLUMN revenuecat_subscriber_get_enabled INTEGER NOT NULL DEFAULT 0")
    connection.execute(
        """
...
def approved_login_from_records(site: sqlite3.Row, profile: sqlite3.Row, interaction: sqlite3.Row) -> ApprovedLogin:
    ...
    return ApprovedLogin(
        ...
        firestore_listen_get_enabled=bool(journey.get("firestore_listen_get_enabled", False)),
        revenuecat_subscriber_get_enabled=bool(journey.get("revenuecat_subscriber_get_enabled", False)),
    )
...
async def get_login_journey(site_id: str) -> dict:
    ...
    return {
        ...
        "firestore_listen_get_enabled": bool(row["firestore_listen_get_enabled"]),
        "revenuecat_subscriber_get_enabled": bool(row["revenuecat_subscriber_get_enabled"]),
        "approved_at": row["approved_at"],
    ...
    with closing(connect_database()) as connection:
        ...
        connection.execute(
            ...
            ON CONFLICT(site_id) DO UPDATE SET
                ...
                firestore_listen_get_enabled = excluded.firestore_listen_get_enabled,
                revenuecat_subscriber_get_enabled = excluded.revenuecat_subscriber_get_enabled,
            ...
            (
                ...
                int(journey.firestore_listen_get_enabled),
                int(journey.revenuecat_subscriber_get_enabled),
            ),
        )
        connection.commit()
    return {
        ...
        "firestore_listen_get_enabled": journey.firestore_listen_get_enabled,
        "revenuecat_subscriber_get_enabled": journey.revenuecat_subscriber_get_enabled,
...
@app.post("/api/sites/{site_id}/fred-online-check-once", tags=["authentication"])
async def run_fred_online_check_once(site_id: str, request: LoginExecutionRequest) -> dict:
    if not request.execution_confirmed:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Explicit confirmation of the single Fred online check is required.")
    definition_json = json.dumps(FRED_ONLINE_CHECK, sort_keys=True)
    ...
    with closing(connect_database()) as connection:
        ...
        if site is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Site not found.")
        if profile is None or login is None or prior_run is None:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="The prior probe audit chain and approved authentication are required.")
        username = os.environ.get(profile["username_env"])
        password = os.environ.get(profile["password_env"])
        if not username or not password:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Configured test credentials are unavailable; the online check was not reserved or submitted.")
        prior_probe = connection.execute("SELECT * FROM interaction_definitions WHERE id = ?", (prior_run["probe_interaction_definition_id"],)).fetchone()
        online_probe = connection.execute("SELECT * FROM interaction_definitions WHERE site_id = ? AND interaction_type = 'chat_probe' AND definition_json = ?", (site_id, definition_json)).fetchone()
        if online_probe is None:
            online_probe_id = str(uuid4())
            connection.execute(
                "INSERT INTO interaction_definitions (id, site_id, interaction_type, version, definition_json, approved_at, supersedes_id) VALUES (?, ?, 'chat_probe', 4, ?, ?, ?)",
                (online_probe_id, site_id, definition_json, started_at, prior_probe["id"]),
            )
            online_probe = connection.execute("SELECT * FROM interaction_definitions WHERE id = ?", (online_probe_id,)).fetchone()
        try:
            connection.execute(
                "INSERT INTO chat_probe_runs (id, site_id, login_interaction_definition_id, probe_interaction_definition_id, started_at, completed_at, status, evidence_json) VALUES (?, ?, ?, ?, ?, ?, 'RUNNING', '{}')",
                (run_id, site_id, login["id"], online_probe["id"], started_at, started_at),
            )
            connection.commit()
        except sqlite3.IntegrityError as error:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="The Fred online check has already been consumed; it cannot run again.") from error
    evidence = await execute_approved_login(
        approved_login_from_records(site, profile, login), username, password,
        collect_control_inventory=True, chat_probe=FRED_ONLINE_CHECK, capture_latest_response=True,
    )
